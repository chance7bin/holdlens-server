...
> `portfolio`、`funddata`、`processing` 的领域边界继续有效；旧账户/持仓表模型已由 [ADR-005](adr-005-asset-record-and-watchlist-model.md) 替代。
...
- `asset_catalog`、`asset_record`、`asset_record_change`：资产目录、当前资产记录和只追加变更历史。
- `watchlist_item`：用户对既有公共基金或股票的独立自选关系。
