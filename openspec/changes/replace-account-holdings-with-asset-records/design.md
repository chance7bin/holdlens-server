...
- 不在本变更设计客户端交互；系统目录只固定后端初始化层级和可录入边界。
...
- 系统初始化创建一级加项分组“投资资产”（`INVESTMENT_ASSET`），并在其下创建二级叶子目录“基金”（`FUND`）和“股票”（`STOCK`）；“投资资产”只分组汇总，不允许直接创建资产记录。
...
当前无待确认事项。客户端如何展开、跳转或呈现“投资资产”分组留到后续交互设计，不影响本次后端目录身份和层级。
