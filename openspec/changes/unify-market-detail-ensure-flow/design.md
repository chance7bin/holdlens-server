...
9. 股票全市场行情按市场批量刷新。Trigger 使用市场时区 cron，Case 根据配置化休市日、提前收盘和交易时段判断是否派发；活动标的不影响该批量任务。
10. `stock_market.refreshed_at` 对外命名为 `quoteFetchedAt`；`quoteAsOf` 只在未来取得交易所/来源行情时间时赋值。
